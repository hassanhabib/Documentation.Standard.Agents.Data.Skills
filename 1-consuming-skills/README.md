...
Resolve what to load and rank it → do it → surface the result. Dependency → Purpose → Exposure.
...
| Same priority, different level | The more specific / more local level wins. Deterministic. |
| Same priority, same level, **composable** | Do both. Not a collision. |
| Same priority, same level, **redundant** (same ask, same way) | Dedupe / soft-note. Not a collision. |
| Same priority, same level, **contradictory** (same ask, opposite ways) | **Genuine tie → escalate.** |

The agent must first *classify* — composable, redundant, or contradictory — and only a
...
A live collision is also an **Output** (§1.2) event: it feeds back to the Managing
